...
const play = (delta) => {
	...
	blobs.forEach((blob, i) => {
		blob.y += blob.vy;
		blob.x += blob.vx;
		let blobHitsWall = contain(blob.getBounds(), {
			x: 28,
			y: 20,
			width: 488,
			height: 480
		...
		if (blobHitsWall === "left" || blobHitsWall === "right") {
			blob.rotation += Math.PI - getRadian({
				x: 0,
				y: 0,
				width: 0,
				height: 0
	...
	bullets.forEach((bullet, i) => {
		...
		bullet.x += bullet.vx;
		//子弹运动范围及触壁反弹
		let bulletHitsWall = contain(bullet.getBounds(), {
			x: 28,
			y: 20,
			width: 488,
			height: 480
		...
		if (bulletHitsWall === "left" || bulletHitsWall === "right") {
			bullet.rotation += Math.PI - getRadian({
				x: 0,
				y: 0,
				width: 0,
				height: 0
...
const createBullets = (point) => {
	...
	bullet.vx = 5 * Math.cos(pointRadian);
	bullet.vy = 5 * Math.sin(pointRadian);
	bullets.push(bullet);
